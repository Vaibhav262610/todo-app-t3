...
const page = async () => {
  ...
  return (
    <div className='h-screen flex flex-col justify-center items-center gap-5 text-black bg-black '>
      ...
      <Link href={!session ? '/sign-in' : '/dashboard'}>
      {!session ? <Button className='bg-red-300 hover:bg-green-500'>Sign In</Button> : <Button className='bg-red-300 hover:bg-green-500'>Dashboard</Button>}
      </Link>
    </div>
  )
}
